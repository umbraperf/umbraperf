use crate::{exec::basic::{basic, count, filter, kpis, uir::{get_top_functions, get_top_srclines, uir}}, record_batch_util::send_record_batch_to_js, state::state::{get_file_size, get_query_from_cache, insert_query_to_cache}, utils::{print_to_cons::print_to_js_with_obj, record_batch_util::concat_record_batches, string_util::{split_at_and, split_at_comma, split_at_double_and, split_at_question_mark, split_at_to}}};
...
fn eval_operations(mut record_batch: RecordBatch, op_vec: Vec<&str>) -> Option<RecordBatch> {
    for op in op_vec {
        ...
        match operator {
            ...
            "top(func)" => {
                record_batch = get_top_functions();
            }
            "top(srclines)" => {
                record_batch = get_top_srclines(0);
            }
            _ => {
