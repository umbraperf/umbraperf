...
impl RecordBatchUtil {
    ...
    pub fn write_record_batches_to_cursor(record_batches: Vec<RecordBatch>) -> Cursor<Vec<u8>> {
        ...
        for batch in record_batches {
            ...
            let encoded_schema = arrow::ipc::writer::IpcDataGenerator::schema_to_bytes(&arrow::ipc::writer::IpcDataGenerator::default(), &Arc::new(batch.schema()),  &options);
            let encoded_message = arrow::ipc::writer::IpcDataGenerator::encoded_batch(&arrow::ipc::writer::IpcDataGenerator::default(), &batch,  &mut dict, &options);
