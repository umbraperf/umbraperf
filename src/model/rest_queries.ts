...
export type QueryVariant =
    ...
    | RestQuery<RestQueryType.GET_OPERATOR_FREQUENCY_PER_EVENT, { event: string, pipelines: string }>
    ...
    | RestQuery<RestQueryType.GET_ABS_OP_DISTR_PER_BUCKET_PER_MULTIPLE_PIPELINES, { event: string, time: string, pipelines: string }>
...
export function createRestQuery(query: QueryVariant) {
    ...
    switch (query.type) {
        ...
        case RestQueryType.GET_ABS_OP_DISTR_PER_BUCKET_PER_MULTIPLE_PIPELINES:
            console.log(`bucket/operator/absfreq/?ev_name="${query.data.event}"/?pipeline="${query.data.pipelines}"/absfreq?pipeline,time:${query.data.time}`);
            return `bucket/operator/absfreq/?ev_name="${query.data.event}"/?pipeline="${query.data.pipelines}"/absfreq?pipeline,time:${query.data.time}`;
